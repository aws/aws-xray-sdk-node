...
export function captureFunc(name: string, fcn: (subsegment: Subsegment) => void, parent?: SegmentLike): void;

export function captureAsyncFunc(name: string, fcn: (subsegment: Subsegment) => void, parent?: SegmentLike): void;

export function captureCallbackFunc<A extends any[]>(name: string, fcn: (...args: A) => void, parent?: SegmentLike): (...args: A) => void;

export function capturePromise(): void;

export type CapturedAWSClient<C extends AWSClient> = { [K in keyof C]:
  C[K] extends AWSRequestMethod<infer P, infer D>
  ? AWSRequestMethod<P & { XRaySegment?: SegmentLike }, D>
  : C[K]
};

export type CapturedAWS<T = typeof AWS> = { [K in keyof T]:
